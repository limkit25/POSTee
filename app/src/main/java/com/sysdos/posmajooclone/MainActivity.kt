...
class MainActivity : AppCompatActivity() {
    ...
    private fun fetchSettings() {
        RetrofitClient.instance.getSettings().enqueue(object : Callback<SettingResponse> {
    ...
    private fun sendTransactionToServer(method: String, cash: Double, change: Double) {
        ...
        RetrofitClient.instance.createTransaction(req).enqueue(object : Callback<Any> {
            override fun onResponse(call: Call<Any>, response: Response<Any>) {
                if (response.isSuccessful) {
                    if (method == "TUNAI") localCashSales += tempGrandTotal else localNonCashSales += tempGrandTotal
                    showReceiptDialog(inv, method, cash, change)
                } else Toast.makeText(applicationContext, "Gagal Transaksi", Toast.LENGTH_SHORT).show()
            }
            override fun onFailure(call: Call<Any>, t: Throwable) { Toast.makeText(applicationContext, "Error Server", Toast.LENGTH_SHORT).show() }
        })
    }
    ...
    private fun showReceiptDialog(invoice: String, method: String, cash: Double, change: Double) {
        val dialogView = LayoutInflater.from(this).inflate(R.layout.dialog_receipt, null)
        val dialog = AlertDialog.Builder(this).setView(dialogView).create(); dialog.setCancelable(false)

        // Bind Standard Views
        dialogView.findViewById<TextView>(R.id.tvRcInvoice).text = invoice
        dialogView.findViewById<TextView>(R.id.tvRcCashier).text = cashierName
        dialogView.findViewById<TextView>(R.id.tvRcTotal).text = formatRupiah(tempGrandTotal)
        dialogView.findViewById<TextView>(R.id.tvRcChange).text = formatRupiah(change)

        // Label Pembayaran (Tunai/QRIS)
        val tvPayLabel = dialogView.findViewById<TextView>(R.id.tvRcPayLabel)
        val tvPayValue = dialogView.findViewById<TextView>(R.id.tvRcPay)
        if (method == "TUNAI") {
            tvPayLabel.text = "Tunai"
            tvPayValue.text = formatRupiah(cash)
        } else {
            tvPayLabel.text = "Bayar via"
            tvPayValue.text = method
        }

        // === BINDING VIEW BARU (Subtotal, Tax, Disc) ===
        ...
        val llDisc = dialogView.findViewById<LinearLayout>(R.id.layoutRcDiscount)

        val tvTax = dialogView.findViewById<TextView>(R.id.tvRcTax)
        val llTax = dialogView.findViewById<LinearLayout>(R.id.layoutRcTax)
        ...
        if (tempDiscount > 0) {
            llDisc.visibility = View.VISIBLE
            tvDisc.text = "- ${formatRupiah(tempDiscount)}"
        } else {
            llDisc.visibility = View.GONE
        }
        ...
        dialogView.findViewById<TextView>(R.id.tvRcItems).text = sb.toString()

        dialogView.findViewById<Button>(R.id.btnRcClose).setOnClickListener {
            // Fungsi print bluetooth kamu sudah otomatis support pajak/diskon
            // karena mengambil variabel global tempDiscount & tempTax
            printStrukBluetooth(invoice, method, cash, change)
    ...
    private fun printStrukBluetooth(invoice: String, method: String, cash: Double, change: Double) {
            ...
            if (connection != null) {
                ...
                try {
                    var text = """
                        ...
                        [L]<b>Tgl:</b>[R]${SimpleDateFormat("dd/MM/yy HH:mm", Locale.getDefault()).format(Date())}
                        [C]--------------------------------
                    """.trimIndent()
                    ...
                    text += "\n[C]--------------------------------"
                    text += "\n[L]Subtotal[R]${formatRupiah(tempSubtotal)}"
                    if (tempDiscount > 0) text += "\n[L]Diskon[R]-${formatRupiah(tempDiscount)}"
                    if (tempTax > 0) text += "\n[L]Pajak ($globalTaxRate%)[R]${formatRupiah(tempTax)}"

                    text += """
                        
                        [L]<b>TOTAL</b>[R]<b>${formatRupiah(tempGrandTotal)}</b>
                ...
                } finally { printer.disconnectPrinter() }
            } else Toast.makeText(this, "Printer tidak ditemukan.", Toast.LENGTH_SHORT).show()
    ...
    private fun startShiftOnServer(m: Double) { RetrofitClient.instance.startShift(StartShiftRequest(userId, m)).enqueue(object : Callback<StartShiftResponse> { override fun onResponse(call: Call<StartShiftResponse>, response: Response<StartShiftResponse>) { if(response.isSuccessful) { currentShiftId = response.body()!!.shift.id; currentInitialCash = m; loadProducts() } }; override fun onFailure(call: Call<StartShiftResponse>, t: Throwable) {} }) }
    private fun showLogoutOptionsDialog() { AlertDialog.Builder(this).setItems(arrayOf("Tutup Toko","Ganti Akun","Batal")){d,w-> when(w){0->showShiftRecapDialog(localCashSales,localNonCashSales); 1->performLogout(); 2->d.dismiss()}}.show() }
    ...
    private fun endShiftOnServer() { RetrofitClient.instance.endShift(EndShiftRequest(currentShiftId)).enqueue(object : Callback<EndShiftResponse> { override fun onResponse(call: Call<EndShiftResponse>, response: Response<EndShiftResponse>) { if(response.isSuccessful) performLogout() }; override fun onFailure(call: Call<EndShiftResponse>, t: Throwable) {} }) }
    ...
    private fun loadProducts() { RetrofitClient.instance.getProducts().enqueue(object : Callback<ProductResponse> { override fun onResponse(call: Call<ProductResponse>, response: Response<ProductResponse>) { if(response.isSuccessful) { fullProductList=response.body()?.data?:emptyList(); productAdapter.updateData(fullProductList); setupCategoryTabs() } }; override fun onFailure(call: Call<ProductResponse>, t: Throwable) {} }) }
